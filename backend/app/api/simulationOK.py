...
JOURS_OUVRES_AN = 264  # jours ouvrés/an
...
def calculer_simulation(
    ...
    *,
    ...
    # 🔹 CORRECTION ICI : meilleure gestion des valeurs par défaut
    colis_amana_par_sac = float(volumes_obj.colis_amana_par_sac) if volumes_obj.colis_amana_par_sac not in [None, ""] else 5.0
    courriers_par_sac = float(volumes_obj.courriers_par_sac) if volumes_obj.courriers_par_sac not in [None, ""] else 4500.0
    ...
    #    - priorité au paramètre idle_minutes
    #    - fallback sur un éventuel champ volumes.idle_minutes (si présent dans VolumesInput)
    idle_min = (
        float(idle_minutes)
    ...
    for t in taches:
        ...
        if is_collecte_colis:
            ...
            if colis_input > 0:
                # 🔹 ici on lit la valeur envoyée par le front
                raw_colis_par_collecte = volumes_obj.colis_par_collecte  # ✅ maintenant présent dans le modèle

                try:
                    colis_par_collecte = float(raw_colis_par_collecte or 0)
                except (TypeError, ValueError):
                    colis_par_collecte = 0

                if colis_par_collecte <= 0:
                    colis_par_collecte = 1.0  # fallback safe

                # 👉 c’est ici que tu dois voir l'effet de "10"
                volume_jour = colis_input / colis_par_collecte
            else:
                volume_jour = 0.0
            print(
                "[DEBUG collecte colis] colis=",
                getattr(volumes_obj, "colis", None),
                "colis_par_collecte=",
                getattr(volumes_obj, "colis_par_collecte", None),
            )




        # 1) COLIS (⚠️ BIEN METTRE `elif` ICI)
        ...
        # 2) SACS
            ...
            if base_sacs_jour > 0:
                ...
                # AMANA : sacs = colis/jour / ratio
                volume_jour = amana_sacs_jour
            elif base_colis_jour > 0:
                # Pas de sacs mais des colis classiques : conversion
