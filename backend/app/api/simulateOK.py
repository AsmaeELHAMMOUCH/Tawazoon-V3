...
# IMPORTANT : on utilise la logique calculer_simulation (backend/services/simulation.py)
from app.services.simulation import calculer_simulation
from app.services.utils import round_half_up, normalize_unit
...
def simulate_effectifs(request: SimulationRequest, db: Session = Depends(get_db)):
    try:
        # 1) tâches
        sql = """
            SELECT t.id, t.nom_tache, t.phase, t.unite_mesure, t.moyenne_min, t.centre_poste_id
                 , cp.poste_id
            FROM dbo.taches t
...
def simulate_vue_centre_optimisee(
    ...
    try:
        ...
        sim_result: SimulationResponse = calculer_simulation(
            ...
            productivite=request.productivite,
            heures_net_input=request.heures_net,
            idle_minutes=getattr(request, "idle_minutes", 0.0),  # 🔹 important
            volumes_annuels=va_dict,
            volumes_mensuels=None,
        )
...
def get_vue_centre_optimisee(
    ...
    idle_minutes: float = 0.0,  # 🔹 NOUVEAU : Idle en GET aussi (pour export CSV)
    ...
    # 🔹 CORRECTION : on passe des volumes ANNUELS ici
    # Le passage en /jour sera fait par _annual_to_daily_post dans simulate_vue_centre_optimisee.
    ...
    # 🔹 On inclut idle_minutes dans la SimulationRequest
...
def simulate_sans_regroupement(request: SimulationRequest, db: Session = Depends(get_db)):
    try:
        sql = """
            SELECT t.id, t.nom_tache, t.phase, t.unite_mesure, t.moyenne_min, t.centre_poste_id
                 , cp.poste_id
            ...
            ORDER BY t.nom_tache
        """
        rows = db.execute(text(sql), {"centre_id": request.centre_id}).mappings().all()
        taches = [dict(r) for r in rows] if rows else []
...
def get_vue_intervenant_details(centre_id: str, db: Session = Depends(get_db)):
    try:
        sql = """
            SELECT 
                cp.id as poste_id,
