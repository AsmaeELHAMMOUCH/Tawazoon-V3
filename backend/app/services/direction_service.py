...
def process_direction_simulation_v2(db: Session, request: DirectionSimRequest) -> DirectionSimResponse:
    print(f"🔹 DEBUT process_direction_simulation id={request.direction_id} mode={request.mode}")
    try:
        ...
        print(f"🔹 Direction trouvée: {direction['label']}")

        # 2) Identify Scoped Centres
        if direction["id"] == 9999:
             sql_centres = "SELECT c.id, c.label FROM dbo.centres c WHERE c.region_id = :rid"
             c_rows = db.execute(text(sql_centres), {"rid": direction["region_id"]}).mappings().all()
        else:
            sql_centres = """
                SELECT c.id, c.label
                FROM dbo.centres c
        ...
        print(f"🔹 {len(centres_map)} centres trouvés")
        ...
        if centre_ids_to_simulate:
            # A. Postes Info
            sql_postes = """
                SELECT cp.id as centre_poste_id, cp.centre_id, p.code, p.nom_poste, p.type_poste, cp.effectif_actuel
        ...
        for cid in centre_ids_to_simulate:
            ...
            if vol_input:
                # Source: Excel / Input
                # Utilisation sécurisée de getattr pour éviter les AttributeError
                raw_volumes = {
                    ...
                    "colis_amana_par_sac": float(getattr(vol_input, "colis_amana_par_sac", None)) if getattr(vol_input, "colis_amana_par_sac", None) is not None else None,
                    "courriers_par_sac": float(getattr(vol_input, "courriers_par_sac", None)) if getattr(vol_input, "courriers_par_sac", None) is not None else None,
                    "colis_par_collecte": float(getattr(vol_input, "colis_par_collecte", None)) if getattr(vol_input, "colis_par_collecte", None) is not None else None,
                }
            elif ref_input:
                 # Source: Database Ref
                raw_volumes = {
            ...
            # Appliquer la conversion : Sacs/Colis sont JOURNALIERS, on ne divise pas par 264.
            caps = raw_volumes.get("colis_amana_par_sac")
            if caps is None: caps = 5.0
            
            cps = raw_volumes.get("courriers_par_sac")
            if cps is None: cps = 4500.0
            
            cpc = raw_volumes.get("colis_par_collecte")
            if cpc is None: cpc = 1.0

            volumes_dict = {
                "sacs": raw_volumes.get("sacs", 0),
                "colis": raw_volumes.get("colis", 0),
                "scelle": raw_volumes.get("scelle", 0),
                "colis_amana_par_sac": float(caps),
                "courriers_par_sac": float(cps),
                "colis_par_collecte": float(cpc),
            ...
            try:
                # Appel simulation avec heures_net_input=None pour calcul interne
                ...
                print(f"Erreur interne simulation: {e}")
                raise HTTPException(
                    status_code=422,
        ...
        print(f"🔥 FATAL ERROR in process_direction_simulation: {e}")
        traceback.print_exc()
        raise