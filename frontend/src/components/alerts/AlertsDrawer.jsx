...
// Drawer principal (intégré au layout, sans overlay)
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        onClick={closeDrawer}
                        className="fixed inset-0 bg-slate-900/20 backdrop-blur-md z-[9998] cursor-pointer"
                    />
                )}
            </AnimatePresence>

            {/* Drawer fixe (ne pousse pas le contenu) */}
            <AnimatePresence>
                {isOpen && (
                    <motion.aside
                        initial={{ x: "100%" }}
                        ...
                        className="fixed right-0 top-0 h-full w-full sm:w-[420px] bg-white border-l-4 border-slate-300 shadow-2xl z-[9999] flex flex-col"
                    >
                        {/* === 1. TITRE + COMPTEUR === */}
