...
export default function VueIntervenant({
  ...
  heuresNet,
  setHeuresNet,
  // 🔹 marge d’inactivité envoyée au backend (optionnelle)
  idleMinutes = 0,
  // 🔹 callback parent qui fera le POST API
  onSimuler,
  display,
  setDisplay,
  refDisplay,
  ...
  const safeColisAmanaParSac = () =>
    Math.max(1, parseNonNeg(colisAmanaParSac) ?? 5);
  ...
  function nombreUniteParUnite(unite, taskName, taskData = {}) {
    ...
    const dailyAmanaSacs = dailyAmanaColis / safeColisAmanaParSac();
    ...
    if (uRaw.includes("sac")) {
      ...
      if (uRaw.includes("courrier")) {
        ...
        return annualCourrier / JOURS_OUVRES_AN / (parseNonNeg(courriersParSac) ?? 4500);
      }
      ...
      const colisInput = parseNonNeg(colis) ?? 0;
      if (colisInput > 0) return colisInput / safeColisAmanaParSac();
  ...
  // 🔹 ICI : on construit un SimulationRequest-like et on le passe au parent
  const handleSimuler = () => {
    ...
    const body = {
      ...
      volumes: {
        sacs: 0, // pas de saisie de sacs ici
        colis: colisInput,
        colis_amana_par_sac: safeColisAmanaParSac(),
        courriers_par_sac: parseNonNeg(courriersParSac) ?? 4500,
        colis_par_collecte: ratioCollecte,
      },

      volumes_annuels: {
        courrier_ordinaire: annualIfAllowed("courrierOrdinaire"),
        courrier_recommande: annualIfAllowed("courrierRecommande"),
        ebarkia: annualIfAllowed("ebarkia"),
        lrh: annualIfAllowed("lrh"),
        amana: annualIfAllowed("amana"),
      },
    };
