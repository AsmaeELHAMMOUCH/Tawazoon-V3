...
const CARD_CLASS = "bg-white rounded-lg border border-slate-200 shadow-sm p-1.5 flex flex-col h-[170px]";
...
export default function DirectionDonutsRow({ centres = [], charts = null }) {
    ...
    const moiModOption = React.useMemo(() => {
        ...
        return {
            ...
            legend: { bottom: '0%', left: 'center', icon: 'circle', itemHeight: 6, textStyle: { fontSize: 8 }, padding: 0 },
    ...
    const barGapOption = React.useMemo(() => {
        ...
        if (charts && charts.top && charts.top.length > 0) {
            ...
            categories = sorted.map(c => c.label?.substring(0, 8) + '..');
            values = sorted.map(c => c.ecart);
        }

        return {
            tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
            grid: { top: '8%', left: '2%', right: '2%', bottom: '2%', containLabel: true },
            xAxis: {
                type: 'category',
                data: categories,
                axisTick: { alignWithLabel: true },
                axisLabel: { interval: 0, rotate: 20, fontSize: 7, overflow: 'truncate', width: 35 }
            },
            yAxis: { type: 'value', axisLabel: { fontSize: 7 }, splitLine: { show: false } },
            series: [{
                name: 'Écart',
                type: 'bar',
                barWidth: '50%',
    ...
    return (
        <div className="grid grid-cols-1 gap-2 mt-2">
            <div className={CARD_CLASS}>
                <h4 className="text-[9px] font-bold text-slate-700 uppercase mb-0.5 px-0.5">Répartition MOI / MOD</h4>
                <div className="flex-1 min-h-0">
                    <ReactECharts option={moiModOption} style={{ height: '100%', width: '100%' }} />
                </div>
            </div>
            <div className={CARD_CLASS}>
                <h4 className="text-[9px] font-bold text-slate-700 uppercase mb-0.5 px-0.5">Top Écarts</h4>
                <div className="flex-1 min-h-0">
