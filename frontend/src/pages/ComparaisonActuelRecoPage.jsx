...
import { simulerComparaisonActuelReco, exportComparaisonActuelReco } from "../api/recommande";
...
export default function ComparaisonActuelRecoPage() {
    ...
    const handleSimuler = async () => {
        ...
        try {
            ...
            const data = await simulerComparaisonActuelReco(payload);
            setOutputs({
                dossiers_jour: Number(data.dossiers_jour).toFixed(1),
                heures_net: Number(data.heures_net).toFixed(2)
            });
            setRows(data.rows || []);
            setTotaux(data.totaux || null);
            setShowGraph(false);
            if (!data.rows?.length) {
                setStatus("Simulation effectuée mais aucune donnée n'a été retournée.");
            }
        } catch (error) {
            console.error(error);
            setRows([]);
    ...
    const handleExport = async () => {
        ...
        try {
            const params = {
                sacs_jour: inputs.sacs_jour,
                dossiers_mois: inputs.dossiers_mois,
                productivite_pct: inputs.productivite_pct
            };
            const blob = await exportComparaisonActuelReco(params);
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = "comparaison_effectifs_recommande.xlsx";
            a.click();
    ...
    const graphData = useMemo(() => {
        ...
        const reco = rows.map((row) => row.recommande);
        const actual = rows.map((row) => row.effectif_actuel);
        if (totaux) {
            labels.push("TOTAL");
            reco.push(totaux.recommande);
            actual.push(totaux.effectif_actuel);
        }
        return {
            labels,
            datasets: [
                { label: "Effectif Recommandé", data: reco, backgroundColor: "#1e3a8a", borderRadius: 6 },
    ...
    const downloadPng = () => {
        ...
        try {
            ...
            link.download = "comparaison-recommande-global.png";
    ...
    return (
        <EsignLayout activeKey="Comparatif Positions recommandée">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6 animate-in fade-in duration-500">
                ...
                <div className="relative overflow-hidden bg-white rounded-xl shadow-sm border border-slate-200/60 p-4">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center flex-col md:flex-row gap-3 text-center md:text-left">
                            ...
                            <div>
                                <h1 className="text-xl font-black tracking-tight text-slate-900 flex items-center justify-center md:justify-start gap-3">
                                    ...
                                    <span className="bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700 text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border border-blue-200 shadow-sm translate-y-px">Recommandé</span>
                                </h1>
                                <p className="text-slate-500 mt-0.5 text-xs max-w-xl">
                                    Différence entre effectif actuel et besoin recommandé basé sur les nouvelles normes.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={() => navigate("/dimensionnement-recommande/global")}
                ...
                <div className="relative py-2">
                    ...
                    <div className="absolute top-1/2 left-0 h-0.5 bg-blue-500 -translate-y-1/2 rounded-full transition-all duration-700 ease-out" style={{ width: totaux ? "100%" : "33%" }} />
                    <div className="relative z-10 flex justify-between max-w-sm mx-auto w-full">
                        <div
                            className={`flex flex-col items-center gap-2 transition-transform duration-300 ${!totaux ? "scale-105 cursor-pointer" : "scale-95 opacity-70 group"}`}
                            onClick={totaux ? resetSimulation : undefined}
                        >
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-lg transition-colors duration-500 ${!totaux ? "bg-blue-600 text-white shadow-blue-500/25 ring-2 ring-blue-100" : "bg-green-500 text-white shadow-green-500/25 ring-2 ring-green-100"}`}>
                                {!totaux ? <Settings size={18} /> : <Activity size={18} />}
                            </div>
                        ...
                        <div
                            className={`flex flex-col items-center gap-2 transition-all duration-500 ${totaux ? "scale-110 opacity-100" : "scale-95 opacity-40 grayscale"}`}
                        >
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-lg transition-colors duration-500 ${totaux ? "bg-blue-600 text-white shadow-blue-500/25 ring-2 ring-blue-100" : "bg-slate-100 text-slate-400"}`}>
                                <Activity size={18} />
                            </div>
                            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-600 bg-white px-2 py-0.5 rounded-full shadow-sm border border-slate-100">
                                2. Résultats
                ...
                {!totaux && (
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden ring-1 ring-slate-900/5 transition-all duration-500">
                        ...
                        <div className="p-4 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                            ...
                            <Stat label="Heures net / jour" value={fmtDecimal(outputs.heures_net)} />
                        </div>
                        ...
                        <div className="bg-white rounded-xl shadow-md border border-slate-200 flex flex-col min-h-[520px] overflow-visible ring-1 ring-blue-500/10 relative animate-in slide-in-from-bottom-8 fade-in duration-700 ease-out fill-mode-backwards">
                            ...
                            <div className="px-5 py-3 border-b border-slate-100 bg-white flex items-center justify-between sticky top-0 z-20 shadow-sm">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="font-bold text-slate-900 text-xs uppercase tracking-wide">Résultats Comparatifs</h3>
                                </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-slate-50/90 backdrop-blur-sm sticky top-0 z-10 text-[10px] font-bold text-slate-500 uppercase tracking-wider shadow-sm">
                                        <tr>
                                            ...
                                            <th className="px-4 py-2 text-right border-b border-slate-100">Recommandé</th>
                                            <th className="px-4 py-2 text-right border-b border-slate-100">Écart</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-50 text-xs">
                                        {rows.map((row, idx) => (
                                            <tr key={idx} className="group hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-4 py-2 text-right font-mono text-slate-600 font-bold">{row.recommande}</td>
                                                <td className={`px-4 py-2 text-right font-mono font-bold ${row.ecart < 0 ? 'text-red-500' : row.ecart > 0 ? 'text-emerald-500' : 'text-slate-600'}`}>
                                        ...
                                        {totaux && (
                                            <tr className="bg-blue-50/80 border-t-2 border-blue-100 text-blue-900 font-black sticky bottom-0 backdrop-blur-md">
                                                ...
                                                <td className="px-4 py-3 text-right font-mono text-xs">{fmtVol(totaux.effectif_actuel)}</td>
                                                <td className="px-4 py-3 text-right font-mono text-xs text-blue-700">{totaux.recommande}</td>
                                                <td className={`px-4 py-3 text-right font-mono text-sm ${totaux.ecart < 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                                                    {totaux.ecart > 0 ? `+${totaux.ecart}` : totaux.ecart}
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
                ...
                {status && (
                    <div className="text-sm text-slate-600 bg-white/80 border border-slate-200 rounded-2xl py-3 px-4 shadow-sm flex items-center gap-2">
            ...
            <div className="flex flex-col items-center gap-4 px-6 py-4 border-b border-slate-100 bg-white md:flex-row md:justify-between md:items-center">
                <div className="flex-1 min-w-[220px] text-center">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-[10px] text-slate-500 uppercase tracking-[0.3em] md:flex-nowrap md:gap-3">
                        <span className="whitespace-nowrap">Comparatif Recommandé</span>
