...
import { fetchNormesRecommandees, exportNormesRecommandeesCsv } from "../api/recommande";
import {
    Download,
    Calculator,
    Search,
...
export default function NormesDimensionnementRecommandeesPage() {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const data = await fetchNormesRecommandees();
            setRows(data || []);
            setStatus("");
        } catch (error) {
            console.error(error);
    ...
    const handleExport = async () => {
        ...
        try {
            const blob = await exportNormesRecommandeesCsv();
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = "normes_dimensionnement_recommande.xlsx";
            a.click();
    ...
    return (
        <EsignLayout activeKey="Normes de dimensionnement recommandées">
                        ...
                        <div className="flex items-center flex-col md:flex-row gap-3 text-center md:text-left">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 mt-0.5 text-xs max-w-xl">
                                    Référentiel des temps unitaires par activité (Processus Recommandé).
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[calc(100vh-140px)] min-h-[500px]">
                    ...
                    <div className="border-b border-slate-100 px-4 py-3 bg-slate-50/50 flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={() => navigate("/dimensionnement-recommande/capacite-nominale")}
                    ...
                    <div className="flex-1 overflow-auto bg-slate-50/50 relative">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-white sticky top-0 z-10 shadow-sm">
                                <tr>
                                    <th className="px-5 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200 w-[50%]">
                                        <div className="flex items-center gap-2">
                                    ...
                                    <tr>
                                        <td colSpan={5} className="px-6 py-20 text-center text-slate-400 animate-pulse">
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <p className="text-[10px] font-medium">Chargement des normes recommandées...</p>
                                            </div>
                                        </td>
                                    </tr>
