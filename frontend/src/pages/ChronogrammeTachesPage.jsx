...
import {
  fetchChronogrammeTaches,
  exportChronogrammeTachesCsv,
  refreshChronogrammeCache,
} from "../api/chronogramme";
...
export default function ChronogrammeTachesPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const payload = await fetchChronogrammeTaches();
  ...
  const handleExport = async () => {
    ...
    try {
      await refreshChronogrammeCache();
      const response = await exportChronogrammeTachesCsv();
      ...
      anchor.href = url;
      anchor.download = "chronogramme_taches.csv";
      anchor.click();
  ...
  return (
    <EsignLayout activeKey="Chronogramme de Traitement Unitaire">
            ...
            <div className="flex items-center flex-col md:flex-row gap-3 text-center md:text-left">
              ...
              <div>
                ...
                <p className="text-slate-500 mt-0.5 text-xs max-w-xl">
                  Détail chronologique des tâches et temps cumulés.
                </p>
              </div>
            </div>
