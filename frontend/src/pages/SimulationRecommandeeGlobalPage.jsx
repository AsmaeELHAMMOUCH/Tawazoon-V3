...
import { simulerGlobalRecommande, exportGlobalRecommandeCsv, getGlobalRecommandeGraph } from "../api/recommande";
...
export default function SimulationRecommandeeGlobalPage() {
    const navigate = useNavigate();
    const [values, setValues] = useState({
        nb_sac: 50,
        nb_dossier_mensuel: 6500,
        productivite: 100,
    });
    ...
    const [graphDataMode, setGraphDataMode] = useState([]);
    const chartInstanceRef = useRef(null);

    // Live calculation for readonly fields
    const dossiersJour = values.nb_dossier_mensuel / 22;
    const hNetJour = (8 * values.productivite) / 100;

    const handleInputChange = (e) => {
        const { name, value } = e.target;
        setValues((p) => ({ ...p, [name]: Number(value) || 0 }));
    };

    const validateInputs = () => {
        if (values.productivite <= 0) {
            setStatus("Les paramètres de productivité doivent être > 0.");
            return false;
        }
        if (values.nb_dossier_mensuel < 0 || values.nb_sac < 0) {
            setStatus("Les volumes ne peuvent pas être négatifs.");
            return false;
        }
        return true;
    };
    ...
    const simulate = async () => {
        ...
        setLoading(true);
        try {
            const data = await simulerGlobalRecommande(values);
            setSimResult(data);
        } catch (e) {
            console.error(e);
            setStatus(e.response?.data?.detail || "Erreur lors de la simulation.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const doExport = async () => {
        if (!simResult?.simulation_id) return;
        try {
            const blob = await exportGlobalRecommandeCsv(simResult.simulation_id);
            const url = window.URL.createObjectURL(blob);
            const link = document.createElement("a");
            link.href = url;
            link.setAttribute("download", `simulation_recommandee_globale_${simResult.simulation_id}.csv`);
    ...
    const openGraph = async () => {
        if (!simResult?.simulation_id) return;
        try {
            const data = await getGlobalRecommandeGraph(simResult.simulation_id);
            setGraphDataMode(data);
            setShowGraph(true);
        } catch (e) {
    ...
    const exportGraphPng = () => {
        ...
        try {
            ...
            const link = document.createElement("a");
            link.href = imageUrl;
            link.download = "effectif_recommande_global_graph.png";
            document.body.appendChild(link);
            link.click();
    ...
    return (
        <EsignLayout activeKey="Simulation Globale recommandée">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6 animate-in fade-in duration-500">
                        ...
                        <div className="flex items-center flex-col md:flex-row gap-3 text-center md:text-left">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 mt-0.5 text-xs max-w-xl">
                                    Vision consolidée des effectifs recommandés pour l'ensemble des positions.
                                </p>
                            </div>
                        </div>

                        <button
                            onClick={() => navigate("/dimensionnement-recommande/comparatif")}
                ...
                <div className="relative py-2">
                    ...
                    <div className="relative z-10 flex justify-between max-w-sm mx-auto w-full">
                        ...
                        <div className={`flex flex-col items-center gap-2 transition-transform duration-300 ${!totals ? 'scale-105 cursor-pointer' : 'scale-95 opacity-80 cursor-pointer hover:opacity-100 group'}`} onClick={totals ? resetSimulation : undefined}>
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-lg transition-colors duration-500 ${!totals ? 'bg-blue-600 text-white shadow-blue-500/25 ring-2 ring-blue-100' : 'bg-blue-500 text-white shadow-blue-500/25 ring-2 ring-blue-50 group-hover:bg-blue-600'}`}>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden ring-1 ring-slate-900/5 transition-all duration-500 mb-6 animate-in slide-in-from-top-4 fade-in fill-mode-backwards">
                        ...
                        <div className="p-4 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                            ...
                            <div className="space-y-1">
                                ...
                                <input
                                    type="number"
                                    name="nb_dossier_mensuel"
                                    value={values.nb_dossier_mensuel}
                                    onChange={handleInputChange}
                                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-slate-300 shadow-inner h-[34px] text-center"
                                />
                            </div>
                            ...
                            <div className="space-y-1 opacity-75">
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider pl-1 flex items-center justify-center gap-1 whitespace-nowrap">
                                    <FileText size={10} /> Dossiers / Jours
                                </label>
                ...
                {totals && (
                    <div className="space-y-6">
                        ...
                        {/* Card 2: Résultats */}
                        <div className="bg-white rounded-xl shadow-md border border-slate-200 flex flex-col min-h-[500px] overflow-visible ring-1 ring-blue-500/10 relative animate-in slide-in-from-bottom-8 fade-in duration-700 ease-out fill-mode-backwards">
                                    ...
                                    <tbody className="divide-y divide-slate-50 text-xs">
                                        {rows.map((r, idx) => (
                                            <tr key={idx} className={`group transition-colors ${r.fte > 0 ? "bg-blue-50/20 hover:bg-blue-50/40" : "hover:bg-slate-50"}`}>
                                                <td className="px-4 py-2 font-medium text-slate-700">{r.nom_metier}</td>
                                        ...
                                        {totals && (
                                            <tr className="bg-blue-50/80 border-t-2 border-blue-100 text-blue-900 font-black sticky bottom-0 backdrop-blur-md">
                                                ...
                                                <td className="px-4 py-3 text-right font-mono text-xs">{fmtNb(totals.total_fte)}</td>
                                                <td className="px-4 py-3 text-right font-mono text-sm text-blue-700">{totals.total_fte_arrondi}</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
            ...
            {/* Graph Modal - ChartJS*/}
            {showGraph && graphDataMode.length > 0 && (
                                ...
                                <div className="mt-1 flex flex-col items-center gap-2 text-[11px] text-slate-500 uppercase tracking-[0.3em] md:flex-row md:gap-4">
                                    ...
                                    <div className="flex flex-wrap gap-2 justify-center">
                                        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-200 text-[11px] font-semibold text-slate-700 shadow-sm">
                                            Sacs/Jour : <span className="text-blue-600">{fmtVol(values.nb_sac)}</span>
                                        </span>
                                        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-200 text-[11px] font-semibold text-slate-700 shadow-sm">
                                            Dossiers/Mois : <span className="text-blue-600">{fmtVol(values.nb_dossier_mensuel)}</span>
                                        </span>
                                        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-200 text-[11px] font-semibold text-slate-700 shadow-sm">
                                            Productivité : <span className="text-blue-600">{values.productivite}%</span>
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 p-6 bg-slate-50/50 min-h-[400px] overflow-y-auto">
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm w-full h-[400px]">
                                <Bar
                                    ...
                                    data={{
                                        labels: graphDataMode.map(d => d.name),
                                        datasets: [
                                            {
                                                label: "Effectif Requis",
                                                data: graphDataMode.map(d => d.effectif),
                                                backgroundColor: (ctx) => {
                                                    if (ctx.dataIndex === graphDataMode.length - 1) return "#1e40af"; // Total color (Darker blue)
                                                    return "#3b82f6";
                                                },
                                                borderRadius: 4,
                                                barPercentage: 0.6,
