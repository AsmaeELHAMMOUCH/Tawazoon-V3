...
export default function Dashboard() {
  ...
  const staffingBars = useMemo(() => {
    ...
    const total = staffing.reduce((acc, r) => {
      acc.actuel += Math.round(Number(r.actuel || 0))
      acc.calcule += Math.round(Number(r.calcule || 0))
      acc.recommande += Math.round(Number(r.recommande || 0))
      return acc
            ...
            <table className="min-w-full text-[13px]">
              ...
              <tbody>
                {(centresRows.length ? centresRows : [
                  ...
                  const vActuel = Math.round(Number(r.actuel || 0))
                  const vRecommande = Math.round(Number(r.recommande || 0))
                  const ecart = vRecommande - vActuel
                  return (
                    <tr key={r.id} className="border-t border-slate-100">
                      <td className="py-2 pr-4 font-medium text-slate-900">{r.centre}</td>
                      <td className="py-2 pr-4 text-slate-700">{r.type}</td>
                      <td className="py-2 pr-4">{vActuel}</td>
                      <td className="py-2 pr-4">{vRecommande}</td>
